...
struct Solver{
    ...
    void solve(){
        ...
        vvec(ll) g(R,vec(ll)(R));
        rep(i,R)rep(j,R) cin >> g[i][j];

        vvec(mint) vv(R,vec(mint)(R));

        ll rr = (1<<R);
        rep(st,R){
            vvec(mint) dp(R,vec(mint)(rr));
            dp[st][(1<<st)] = 1;
            rep(x,rr-1){
                rep(i,R)rep(j,R){
                    if(g[i][j]==1 && !bit(x,j)) dp[j][x+(1<<j)] += dp[i][x]; 
                }
            }
            rep(i,R)rep(x,rr) vv[st][i] += dp[i][x];
        }



        ll hh = H;
        vvec(mint) a(R,vec(mint)(R));
        rep(i,R) a[i][i]=1;
        while(hh){
            if(hh&1) a = vvxvv(vv,a);
            vv = vvxvv(vv,vv);
            hh>>=1;
        }
